...
const Selectbtn = document.querySelector('.sr-only');
...
const rangInput = document.querySelectorAll('input');
 for(let i = 0 ; i < rangInput.length ; i++ ) {
     rangInput[i].addEventListener('input' , () => {
         if(rangInput[i].id === "bright"){
             canvas.style.filter = `brightness(${rangInput[i].value}%)`;
         }
      })
  }
  ...
  for(let i = 0 ; i < rangInput.length ; i++ ) {
     rangInput[i].addEventListener('input' , () => {
         if(rangInput[i].id === "contrast"){
             canvas.style.filter = `contrast(${rangInput[i].value}%)`;
         }
      })
  }
